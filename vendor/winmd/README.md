...
that binary in this MIT-licensed public repo isn't appropriate, so these two
...
(its top-level `winmd/` directory — including these same two files — is
populated out of band and is entirely `.gitignore`d there too).
...
new `.nupkg`, e.g. via `Get-FileHash` / `sha256sum`), then rerun the step.
