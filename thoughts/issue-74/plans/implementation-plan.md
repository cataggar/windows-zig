...
**Conclusion**: this is very likely a real, deterministic bug in how `Microsoft.UI.Xaml.dll` handles the **first real layout/render pass** once `XamlControlsResources` has actually been merged and a window is shown in this unpackaged/aggregated bootstrap model — not a reference-counting bug in reactor's own code, and not specific to `TextBox` (`reactor_hello`, which was used for all of this investigation, does not construct a `TextBox` at all). This is squarely the kind of runtime/lifetime problem the plan's "What We're NOT Doing" section already excludes as out of scope (the shutdown leak/crash tracked in issue #60) — though it manifests earlier in the lifecycle (at window activation, not just at final process teardown) than #60's description suggests, so it may be a related-but-distinct issue rather than literally the same one.

**Tracked as issue #86**: [Reactor: crash in Microsoft.UI.Xaml.dll on first window activation once XamlControlsResources is merged (found while fixing #74)](https://github.com/cataggar/windows-zig/issues/86). #74's own scope should not be blocked on resolving #86 — #74's actual reported bug (`TextBox` construction crashing with `0xC000027B`) is fixed by the `IXamlMetadataProvider`-delegation approach in this plan; #86 is a separate, newly-discovered issue that must be resolved (or explicitly worked around) before `TextBox` can be **permanently** re-enabled in `winui_backend.zig`, since re-enabling it means real apps will actually show windows with merged theme resources, which is exactly what triggers #86.

**#86 investigation complete — corrected understanding and blocking scope (2026-07-08)**: the full investigation lives in [`thoughts/issue-86/plans/implementation-plan.md`](../../issue-86/plans/implementation-plan.md) (PR #87). Two findings materially change the above:

1. **The crash is not at first window activation at all.** A symbolized stack trace (via `cdb.exe` + the public Microsoft symbol server) proved it is a **process-teardown bug**: `Application.Start` returns completely normally (the window runs for its full lifetime), and the crash happens later, inside `Microsoft.UI.Xaml.dll`'s own `DllMain(DLL_PROCESS_DETACH)` handler, tearing down its internal `DirectUI::DynamicMetadataStorage` singleton (`ctl::ComPtr<DirectUI::DependencyPropertyHandle>::InternalRelease` reading freed memory). The earlier "crash happens right after `activateWindows()`, before the pump even runs" conclusion above was most likely an artifact of unflushed stdout at crash time, not an actual first-render fault.
2. **`XamlControlsResources`/the resource merge has nothing to do with it.** Bisection proved the identical crash reproduces even with **no `ResourceDictionary` merged at all** — merely having the COM aggregation expose a *real, functioning* `IXamlMetadataProvider` (delegating to the real `XamlControlsXamlMetaDataProvider`) is sufficient on its own. A stub-only "not found" `IXamlMetadataProvider` (no real delegation) does **not** crash.

**Practical consequence**: permanently wiring **this Phase 1** (the aggregation + real-provider-delegation itself, independent of whether Phase 2's resource merge below is ever implemented) is what's actually blocked on #86 — not just Phase 2 as the paragraph above originally assumed. Landing Phase 1 permanently, even without Phase 2, would already crash every reactor app at shutdown, including `reactor-selftest`'s existing `reactor-hello`/`reactor-counter` smoke checks. #86's Phase 3 research further found this is a known, Microsoft-acknowledged limitation (cross-reproduced in Rust/`windows-app-rs` and Swift/`SwiftWinRT` projections, with an explicit "no internal interest in making this work" response from a Windows App SDK-adjacent maintainer) rather than a bug likely to be fixed upstream — so this plan's Phase 1/2 should remain a documented spike (as they already are, gated behind `issue74_use_aggregated_application`) rather than being wired in permanently, until/unless a workaround is found or the risk is explicitly accepted.

**#86 RESOLVED — fix found, no longer blocking (2026-07-08, same day, follow-up)**: the "no internal interest in making this work" conclusion above was superseded within the same investigation. Direct comparison with this repo's own reference implementation — `../windows-rs`'s `crates/libs/reactor/src/app.rs` — showed it never calls `RoUninitialize()`/`CoUninitialize()` anywhere (only `CoInitializeEx` at startup), unlike this Zig port's `packages/win-reactor/src/app.zig`, which had added `defer win_core.winrt.RoUninitialize();` during the initial Rust→Zig port. Removing that call fixes the crash completely: confirmed 5/5 clean runs of `reactor-hello`/`reactor-counter` with the full aggregation + real provider delegation + `XamlControlsResources` merge + real `TextBox` construction all active, plus a clean `zig build test --summary all`. **This Phase 1 (and Phase 2 below) are no longer blocked** — they can be wired in permanently, *provided* `app.zig` also drops its `RoUninitialize()` call the same way (already done in the `issue-86-plan` branch; carry it forward when this plan's own Phase 1/2 land). See `thoughts/issue-86/plans/implementation-plan.md`'s "SUPERSEDED" section for full details.

## Phase 2: Wire aggregation permanently and merge theme resources (only if Phase 1 succeeds)

**No longer blocked on #86 as of 2026-07-08** — see the "#86 RESOLVED" note above. The earlier "BLOCKED on #86" marker here is superseded: a fix (dropping `app.zig`'s `RoUninitialize()` call) was found and validated. This phase's permanent wiring can proceed, carrying that fix forward.
...
- Issue #86 — Reactor: crash in Microsoft.UI.Xaml.dll on first window activation once XamlControlsResources is merged (found while fixing #74; must be resolved before permanently re-enabling `TextBox`)
  <https://github.com/cataggar/windows-zig/issues/86>
