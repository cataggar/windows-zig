...
const bundle_artifacts = @import("bundle_artifacts.zig");
...
/// import alias for each entry, so the cross-namespace qualifiers
/// `writeZigTy` emits resolve against something concrete.
///
...
const ExternalPlaceholder = struct {
    namespace: []const u8,
};

const ExternalPlaceholderSet = std.StringArrayHashMapUnmanaged(ExternalPlaceholder);
...
/// File name emitted by `winbindgen bundle` for the public bundle root.
pub const BundleFacadeFileName = bundle_artifacts.BundleFacadeFileName;

/// File name emitted by `winbindgen bundle` for namespace dependency metadata.
pub const BundleDependenciesFileName = bundle_artifacts.BundleDependenciesFileName;

/// Import rendering strategy for generated full-namespace files.
pub const ImportStyle = enum {
    /// Sibling-file imports: `@import("Windows.Foundation.zig")`.
    relative,
    /// Stable Zig module imports: `@import("Windows.Foundation")`.
    module,
};

/// Allocate the stable Zig module name for a generated namespace.
pub fn namespaceModuleNameAlloc(allocator: std.mem.Allocator, namespace: []const u8) ![]const u8 {
    return allocator.dupe(u8, namespace);
}

fn writeNamespaceModuleName(writer: *std.Io.Writer, namespace: []const u8) !void {
    try writer.writeAll(namespace);
}

fn writeNamespaceImportExpr(writer: *std.Io.Writer, namespace: []const u8, import_style: ImportStyle) !void {
    switch (import_style) {
        .relative => try writer.print("@import(\"{s}.zig\")", .{namespace}),
        .module => {
            try writer.writeAll("@import(\"");
            try writeNamespaceModuleName(writer, namespace);
            try writer.writeAll("\")");
        },
    }
}

fn recordExternalPlaceholder(
    placeholders: *ExternalPlaceholderSet,
    gpa: std.mem.Allocator,
    tn: winmd.TypeName,
) !void {
    const gop = try placeholders.getOrPut(gpa, tn.name);
    if (!gop.found_existing) {
        gop.value_ptr.* = .{ .namespace = tn.namespace };
    }
}

fn emitExternalPlaceholders(
    writer: *std.Io.Writer,
    arena: std.mem.Allocator,
    placeholders: *const ExternalPlaceholderSet,
) !void {
...
fn writeBundleFacadeImport(writer: *std.Io.Writer, lhs: []const u8, namespace: []const u8, import_style: ImportStyle) !void {
    try writer.print("{s} = ", .{lhs});
    try writeNamespaceImportExpr(writer, namespace, import_style);
    try writer.writeAll(";\n");
}
...
fn emitNamespaceExWithImports(
    ...
    var external_placeholders: ExternalPlaceholderSet = .empty;
    ...
    try emitStructsImpl(body_writer, arena, file, namespace, &cross, &external_placeholders, arch, type_entries);
    try emitInterfaceHandlesImpl(body_writer, arena, file, namespace, &cross, &external_placeholders);
    try emitDelegates(body_writer, arena, file, namespace);
    try emitRuntimeClasses(body_writer, arena, file, namespace, &cross);
    try emitInterfaceVtblsImpl(body_writer, arena, file, namespace, &cross, &external_placeholders);
    try emitFreeFunctionsImpl(body_writer, arena, file, namespace, &cross, &external_placeholders, arch);
    ...
    try emitGenericInstantiations(body_writer, arena, file, namespace, &cross, &external_placeholders, &insts);
    ...
    if (dependencies_out) |out| out.* = sorted;
    for (sorted) |ns| {
        try writer.print("const @\"{s}\" = ", .{ns});
        try writeNamespaceImportExpr(writer, ns, import_style);
        try writer.writeAll(";\n");
    }
    try writer.writeAll("\n");
    ...
    try emitExternalPlaceholders(writer, arena, &external_placeholders);
...
test "namespace module imports use stable generated names" {
    const module_name = try namespaceModuleNameAlloc(std.testing.allocator, "Windows.Win32.Foundation");
    defer std.testing.allocator.free(module_name);
    try std.testing.expectEqualStrings("Windows.Win32.Foundation", module_name);

    var buf: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer buf.deinit();
    try writeNamespaceImportExpr(&buf.writer, "Windows.Wdk.System.Memory", .module);
    try std.testing.expectEqualStrings(
        "@import(\"Windows.Wdk.System.Memory\")",
        buf.written(),
    );
}
...
pub fn emitFreeFunctions(
    ...
    var external_placeholders: ExternalPlaceholderSet = .empty;
    try emitFreeFunctionsImpl(writer, arena, file, namespace, cross, &external_placeholders, arch);
}

fn emitFreeFunctionsImpl(
    writer: *std.Io.Writer,
    arena: std.mem.Allocator,
    file: *const winmd.File,
    namespace: []const u8,
    cross: *CrossNsSet,
    external_placeholders: *ExternalPlaceholderSet,
    arch: Arch,
) !void {
    const rows = file.rowCount(.type_def);
    var row: u32 = 0;
    while (row < rows) : (row += 1) {
        ...
        while (m < methods.end) : (m += 1) {
            ...
            if (std.mem.eql(u8, method_name, import_name)) {
                ...
                try writeFnSig(writer, arena, sig, namespace, cross, external_placeholders, true);
                try writer.writeAll(";\n");
            } else {
                ...
                try writeFnSig(writer, arena, sig, namespace, cross, external_placeholders, false);
...
fn writeFnSig(
    ...
    for (sig.params, 0..) |p, i| {
        ...
        try writeParam(writer, arena, p, namespace, cross, external_placeholders);
    }
    try writer.writeAll(") callconv(.winapi) ");
    if (sig.return_type == .void) {
        try writer.writeAll("void");
    } else {
        _ = try writeZigTy(writer, arena, sig.return_type, namespace, cross, external_placeholders, null, null, null);
    }
}
...
pub fn emitInterfaceVtbls(
    ...
    var cross: CrossNsSet = .empty;
    var external_placeholders: ExternalPlaceholderSet = .empty;
    try emitInterfaceVtblsImpl(writer, arena, file, namespace, &cross, &external_placeholders);
}
...
fn emitInterfaceVtblsImpl(
    ...
    while (row < rows) : (row += 1) {
        ...
        while (m < methods.end) : (m += 1) {
            ...
            try writeMethodPointer(writer, arena, file, m, name, namespace, cross, external_placeholders);
            try writer.writeAll(",\n");
        }
...
/// Result of a bundle emission. Namespace entries map input namespaces
/// to fully-emitted Zig source bytes (arena-allocated), and `facade`
/// is the public bundle root written as `BundleFacadeFileName`.
pub const BundleEmission = struct {
    namespaces: std.StringArrayHashMapUnmanaged([]const u8),
    dependencies: std.StringArrayHashMapUnmanaged([]const []const u8),
    facade: []const u8,

    pub fn contains(self: BundleEmission, namespace: []const u8) bool {
        return self.namespaces.contains(namespace);
    }

    pub fn get(self: BundleEmission, namespace: []const u8) ?[]const u8 {
        return self.namespaces.get(namespace);
    }

    pub fn getDependencies(self: BundleEmission, namespace: []const u8) ?[]const []const u8 {
        return self.dependencies.get(namespace);
    }
};

/// Emit the public root module for a generated bundle.
pub fn emitBundleFacade(writer: *std.Io.Writer) !void {
    try emitBundleFacadeWithImports(writer, .relative);
}

/// Emit the public root module for a generated bundle using `import_style`.
pub fn emitBundleFacadeWithImports(writer: *std.Io.Writer, import_style: ImportStyle) !void {
    try writer.writeAll("// Generated by winbindgen - do not edit.\n");
    try writeBundleFacadeImport(writer, "pub const Foundation", "Windows.Win32.Foundation", import_style);
    try writeBundleFacadeImport(writer, "pub const Com", "Windows.Win32.System.Com", import_style);
    try writer.writeAll(
    ...
    try writeBundleFacadeImport(writer, "            pub const Composition", "Windows.Win32.System.WinRT.Composition", import_style);
    try writer.writeAll(
        \\        };
        \\    };
        \\};
    ...
    try writeBundleFacadeImport(writer, "    pub const Foundation", "Windows.Foundation", import_style);
    try writeBundleFacadeImport(writer, "    pub const Collections", "Windows.Foundation.Collections", import_style);
    try writeBundleFacadeImport(writer, "    pub const Numerics", "Windows.Foundation.Numerics", import_style);
    try writeBundleFacadeImport(writer, "    pub const Graphics", "Windows.Graphics", import_style);
    try writeBundleFacadeImport(writer, "    pub const Globalization", "Windows.Globalization", import_style);
    try writer.writeAll("    pub const System = struct {\n        const Windows_System = ");
    try writeNamespaceImportExpr(writer, "Windows.System", import_style);
    try writer.writeAll(
        \\;
    ...
    try writeBundleFacadeImport(writer, "        pub const Threading", "Windows.System.Threading", import_style);
    try writer.writeAll(
        \\    };
        \\    pub const UI = struct {
        \\
    );
    try writer.writeAll("        const Windows_UI = ");
    try writeNamespaceImportExpr(writer, "Windows.UI", import_style);
    try writer.writeAll(
        \\;
    ...
    try writer.writeAll("            const Windows_UI_Composition = ");
    try writeNamespaceImportExpr(writer, "Windows.UI.Composition", import_style);
    try writer.writeAll(
        \\;
    ...
    try writeBundleFacadeImport(writer, "            pub const Desktop", "Windows.UI.Composition.Desktop", import_style);
    try writer.writeAll(
        \\        };
        \\    };
    ...
    try writeBundleFacadeImport(writer, "        pub const Enumeration", "Windows.Devices.Enumeration", import_style);
    try writer.writeAll(
        \\    };
        \\};
        \\
...
///   4. Attach the generated public facade bytes for callers that want
///      to persist the bundle root next to the namespace files.
///
/// The returned `BundleEmission` owns no memory beyond the arena; when
/// the arena is freed all arena-backed buffers vanish. The caller is
/// expected to consume the bytes before arena teardown.
pub fn emitBundle(
    arena: std.mem.Allocator,
    file: *const winmd.File,
    ...
    return emitBundleWithImports(arena, file, namespaces, arch, .relative);
}

/// Emit a bundle of namespaces using `import_style` for cross-namespace
/// aliases and the public bundle facade.
pub fn emitBundleWithImports(
    arena: std.mem.Allocator,
    file: *const winmd.File,
    namespaces: []const []const u8,
    arch: Arch,
    ...
    var facade_buf: std.Io.Writer.Allocating = .init(arena);
    try emitBundleFacadeWithImports(&facade_buf.writer, import_style);
    const facade = facade_buf.written();

    return .{
        .namespaces = out,
        .dependencies = deps_by_ns,
        .facade = facade,
    };
}

/// Emit a small Zig manifest describing generated bundle namespace modules.
pub fn emitBundleDependencyManifest(
    writer: *std.Io.Writer,
    arena: std.mem.Allocator,
    emission: BundleEmission,
) !void {
    const keys = emission.namespaces.keys();
    const sorted = try arena.dupe([]const u8, keys);
    std.mem.sort([]const u8, sorted, {}, struct {
        fn lt(_: void, a: []const u8, b: []const u8) bool {
            return std.mem.lessThan(u8, a, b);
    ...
    for (sorted) |ns| {
        ...
        try writeNamespaceModuleName(writer, ns);
        try writer.writeAll("\", .dependencies = &[_][]const u8{");
        if (emission.getDependencies(ns)) |deps| {
            for (deps, 0..) |dep, i| {
                if (i != 0) try writer.writeAll(", ");
                try writer.writeAll("\"");
    ...
    while (iterations < 64) : (iterations += 1) {
        ...
        for (keys) |mangled_name| {
            ...
            while (m < methods.end) : (m += 1) {
                ...
                try writeSubstitutedMethodPointer(writer, arena, file, m, mangled_name, namespace, cross, external_placeholders, tn.generics);
                try writer.writeAll(",\n");
            }
            try writer.writeAll("};\n");
    ...
    for (params, 0..) |p, i| {
        switch (classifyArrayParam(p, file, method_row, @intCast(i))) {
            .pass => {
                ...
                _ = try writeZigTy(writer, arena, inner.*, current_namespace, cross, external_placeholders, null, null, null);
            },
            .fill => {
                const inner = p.array;
            ...
            .not_array => {
                ...
                try writeParam(writer, arena, p, current_namespace, cross, external_placeholders);
            },
    ...
    if (ret != .void and !isHresultTy(ret)) {
        switch (ret) {
            .array => |inner| {
                ...
                _ = try writeZigTy(writer, arena, ret, current_namespace, cross, external_placeholders, null, null, null);
            },
        }
    }
...
pub fn emitInterfaceHandles(
    ...
    var external_placeholders: ExternalPlaceholderSet = .empty;
    try emitInterfaceHandlesImpl(writer, arena, file, namespace, cross, &external_placeholders);
}

fn emitInterfaceHandlesImpl(
    writer: *std.Io.Writer,
    arena: std.mem.Allocator,
    file: *const winmd.File,
    namespace: []const u8,
    ...
    while (row < rows) : (row += 1) {
        ...
        while (m < methods.end) : (m += 1) {
            ...
            if (gop.found_existing) {
                ...
                try writeMethodWrapper(writer, arena, file, m, name, namespace, cross, external_placeholders, suffixed);
            } else {
                gop.value_ptr.* = 1;
                try writeMethodWrapper(writer, arena, file, m, name, namespace, cross, external_placeholders, null);
            }
        }
...
///       invoke: @FieldType(<Mangled>_Vtbl, "Invoke"),
///       user_data: ?*anyopaque,
///   ) (win_core.hresult.Error || error{OutOfMemory})!EventRegistrationToken
///
///   pub fn removeX(self: *const IFace, token: EventRegistrationToken) win_core.hresult.Error!void
/// ```
///
/// The body wires the user's `invoke` through `win_core.Delegate`,
/// dispatches the raw `add_X(handler, &token)` slot, drops the
/// caller's initial reference (`add_X` AddRefs internally on success),
/// and translates the HRESULT into a Zig error.
///
/// Gating rules — pairs that fail any of these are skipped silently
/// (the raw `add_X`/`remove_X` slots emitted by `writeMethodWrapper`
/// remain available):
...
fn tryEmitEventSugar(
    ...
    for (pairs) |pair| {
        const tn = switch (pair.delegate_param) {
            .class_name => |c| c,
            else => continue,
        };
        ...
        // `remove_` prefixes, so collisions are theoretical but
        // metadata can be surprising — be defensive.
        const add_sugar = try std.fmt.allocPrint(arena, "add{s}", .{pair.event_name});
        const remove_sugar = try std.fmt.allocPrint(arena, "remove{s}", .{pair.event_name});
        if (seen_names.contains(add_sugar) or seen_names.contains(remove_sugar)) continue;

        // Reserve the names so subsequent overload-suffix logic also
        // sees them (sugar emission runs after the methods loop, so
        // this is mostly belt-and-suspenders).
        try seen_names.put(arena, add_sugar, 1);
        try seen_names.put(arena, remove_sugar, 1);
...
fn writeMethodWrapper(
    ...
    if (split_return) {
        switch (sig.return_type) {
            .array => |inner| {
                ...
                try writer.writeAll(", result: *");
                _ = try writeZigTy(writer, arena, sig.return_type, current_namespace, cross, external_placeholders, null, null, null);
            },
        }
    }
    ...
    // `[]const u16` inputs and returns `!win_core.Hstring` by composing
    // the sibling `<Method>FromUtf16` helper with `win_core.hresult.ok`.
    if (has_hstring_in and split_return and sig.return_type == .string and !has_array_param) {
        ...
        try writer.writeAll(") !win_core.Hstring {\n");
        try writer.writeAll("        var r: HSTRING = null;\n");
        try writer.print("        try win_core.hresult.ok(self.{s}FromUtf16(", .{method_name});
        var wrote_arg = false;
        for (sig.params, 0..) |_, i| {
            if (wrote_arg) {
                try writer.writeAll(", ");
...
fn writeParam(
    ...
    switch (ty) {
        .ref_mut => |inner| {
            try writer.writeAll("*");
            _ = try writeZigTy(writer, gpa, inner.*, current_namespace, cross, external_placeholders, null, null, null);
        },
        .ref_const => |inner| {
            try writer.writeAll("*const ");
            _ = try writeZigTy(writer, gpa, inner.*, current_namespace, cross, external_placeholders, null, null, null);
        },
        else => _ = try writeZigTy(writer, gpa, ty, current_namespace, cross, external_placeholders, null, null, null),
    }
}
    ...
    switch (ty) {
        ...
        .class_name => |tn| {
            ...
            if (std.mem.findScalar(u8, projected_tn.name, '`') != null) {
                ...
                try writer.writeAll("*anyopaque");
            } else if (std.mem.eql(u8, projected_tn.namespace, current_namespace) or projected_tn.namespace.len == 0) {
                try writer.print("*{s}", .{projected_tn.name});
            } else if (isProjectableNs(projected_tn.namespace)) {
                try cross.put(gpa, projected_tn.namespace, {});
                try writer.print("*@\"{s}\".{s}", .{ projected_tn.namespace, projected_tn.name });
            } else {
                try recordExternalPlaceholder(external_placeholders, gpa, tn);
                try writer.print("*{s}", .{projected_tn.name});
            }
        },
        ...
        .ptr_mut => |p| {
            ...
            return writeZigTy(writer, gpa, p.inner.*, current_namespace, cross, external_placeholders, renames, entries, file);
        },
        .ptr_const => |p| {
            if (p.inner.* == .void) {
                var d: u32 = 0;
        ...
        .array_fixed => |a| {
            ...
            return writeZigTy(writer, gpa, a.inner.*, current_namespace, cross, external_placeholders, renames, entries, file);
        },
...
pub fn emitStructs(
    ...
    var cross: CrossNsSet = .empty;
    var external_placeholders: ExternalPlaceholderSet = .empty;
    try emitStructsImpl(writer, arena, file, namespace, &cross, &external_placeholders, arch, null);
}
...
pub fn emitStructsFile(
    ...
    try emitStructsImpl(body_writer, arena, file, namespace, &cross, &external_placeholders, arch, null);
...
pub fn collectStructsClosure(
    ...
    while (queue.pop()) |ns| {
        ...
        try emitStructsImpl(&scratch.writer, arena, file, ns, &cross, &external_placeholders, arch, null);
...
fn emitStructsImpl(
    ...
    while (row < rows) : (row += 1) {
        ...
        try emitOneStruct(writer, arena, file, namespace, cross, external_placeholders, row, name, arch, entries, &memo, &all_names);
        try emitted_names.put(arena, name, {});
    }
}
...
fn emitOneStruct(
    ...
    while (i < nc_total) : (i += 1) {
        ...
        try emitOneStruct(writer, arena, file, namespace, cross, external_placeholders, child_row, effective_name, arch, entries, memo, all_names);
    }
    ...
    while (f < field_list.end) : (f += 1) {
        ...
        _ = try writeZigTy(writer, arena, ty, namespace, cross, external_placeholders, if (renames.count() == 0) null else &renames, entries, file);
        try writer.writeAll(",\n");
    }
    try writer.writeAll("};\n");
}
...
test "emitInterfaceHandles writes IStringable handle with method wrappers" {
    ...
    // The combined helper reuses the sibling `FromUtf16` wrapper so
    // HSTRING creation keeps the shared `E_OUTOFMEMORY` HRESULT path.
    try std.testing.expect(std.mem.find(
        u8,
        out,
        "try win_core.hresult.ok(self.UnescapeComponentFromUtf16(p0, &r));",
    ) != null);
...
test "emitNamespace emits addX/removeX sugar for IMemoryBufferReference.Closed" {
    const bytes = @embedFile("Windows.winmd");
    var file = try winmd.parse(bytes);
    ...
    // Sugar method `addClosed` on IMemoryBufferReference: takes an
...
test "emitBundleWithImports emits WinUI placeholders for unsupported external refs" {
    const bytes = @embedFile("Microsoft.UI.Xaml.winmd");
    var file = try winmd.parse(bytes);
    ...
    const out = try emitBundleWithImports(arena.allocator(), &file, &namespaces, .x64, .module);
    const xaml = out.get("Microsoft.UI.Xaml").?;
    const controls = out.get("Microsoft.UI.Xaml.Controls").?;
    ...
    const xaml_placeholder_idx = std.mem.find(u8, xaml, "pub const IResourceManager = opaque {};").?;
    const xaml_signature_idx = std.mem.find(
        u8,
        xaml,
    ...
    try std.testing.expect(xaml_placeholder_idx < xaml_signature_idx);
    try std.testing.expect(std.mem.find(
        u8,
        xaml,
    ...
    try std.testing.expect(std.mem.find(
        u8,
        xaml,
        "const @\"Microsoft.Windows.ApplicationModel.Resources\" =",
    ) == null);

    const controls_placeholder_idx = std.mem.find(u8, controls, "pub const CoreWebView2 = opaque {};").?;
    const controls_signature_idx = std.mem.find(
        u8,
        controls,
        "pub fn get_CoreWebView2(self: *const IWebView2, result: **CoreWebView2) callconv(.winapi) HRESULT {",
    ).?;
    try std.testing.expect(controls_placeholder_idx < controls_signature_idx);
    try std.testing.expect(std.mem.find(
        u8,
        controls,
        "pub const CoreWebView2Environment = opaque {};",
    ) != null);
    try std.testing.expect(std.mem.find(
        u8,
        controls,
        "pub const CoreWebView2ControllerOptions = opaque {};",
    ) != null);
    try std.testing.expect(std.mem.find(
        u8,
        controls,
        "EnsureCoreWebView2Async: *const fn (this: *const IWebView22, p0: *CoreWebView2Environment, result: **@\"Windows.Foundation\".IAsyncAction) callconv(.winapi) HRESULT,",
    ) != null);
    try std.testing.expect(std.mem.find(
        u8,
        controls,
        "@\"EnsureCoreWebView2Async_2\": *const fn (this: *const IWebView22, p0: *CoreWebView2Environment, p1: *CoreWebView2ControllerOptions, result: **@\"Windows.Foundation\".IAsyncAction) callconv(.winapi) HRESULT,",
    ) != null);
    try std.testing.expect(std.mem.find(
        u8,
        controls,
        "const @\"Microsoft.Web.WebView2.Core\" =",
    ) == null);

    for (out.getDependencies("Microsoft.UI.Xaml").?) |dep| {
        try std.testing.expect(!std.mem.eql(u8, dep, "Microsoft.Windows.ApplicationModel.Resources"));
    }
    for (out.getDependencies("Microsoft.UI.Xaml.Controls").?) |dep| {
        try std.testing.expect(!std.mem.eql(u8, dep, "Microsoft.Web.WebView2.Core"));
    }
}

test "emitBundle can render module-name imports and dependency manifest" {
    const bytes = @embedFile("Windows.winmd");
    var file = try winmd.parse(bytes);
    ...
    const out = try emitBundleWithImports(arena.allocator(), &file, &namespaces, .x64, .module);
    const globalization = out.get("Windows.Globalization").?;
    ...
    try std.testing.expect(std.mem.find(
        u8,
        globalization,
        "@import(\"Windows.Foundation.zig\")",
    ) == null);

    const deps = out.getDependencies("Windows.Globalization").?;
    var saw_collections = false;
    for (deps) |dep| {
        if (std.mem.eql(u8, dep, "Windows.Foundation.Collections")) {
            saw_collections = true;
            break;
        }
    }
    try std.testing.expect(saw_collections);
    ...
    try std.testing.expect(std.mem.find(
        u8,
        manifest.written(),
        ".{ .namespace = \"Windows.Globalization\", .module = \"Windows.Globalization\"",
    ) != null);
}
