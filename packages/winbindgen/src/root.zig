...
/// Public relative-import facade mounted by `zig/build.zig` as the
/// `win-bundle` module in compatibility mode.
pub const BundleFacadeSource =
...
pub fn emitBundleFacadeWithImports(writer: *std.Io.Writer, import_style: ImportStyle) !void {
    if (import_style == .relative) {
        try writer.writeAll(BundleFacadeSource);
        return;
    }
...
pub fn emitBundleWithImports(
    ...
    var facade: []const u8 = BundleFacadeSource;
