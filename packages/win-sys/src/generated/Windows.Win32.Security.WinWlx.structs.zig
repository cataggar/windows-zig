// Generated by winbindgen — do not edit.
const win_core = @import("win-core");
const BOOL = win_core.BOOL;
const GUID = win_core.GUID;
const HRESULT = win_core.HRESULT;
const NTSTATUS = win_core.NTSTATUS;
const BOOLEAN = win_core.BOOLEAN;
const @"Windows.Win32.Foundation" = @import("Windows.Win32.Foundation.structs.zig");
const @"Windows.Win32.Security" = @import("Windows.Win32.Security.structs.zig");
const @"Windows.Win32.System.StationsAndDesktops" = @import("Windows.Win32.System.StationsAndDesktops.structs.zig");

pub const WLX_SC_NOTIFICATION_INFO = extern struct {
    pszCard: @"Windows.Win32.Foundation".PWSTR,
    pszReader: @"Windows.Win32.Foundation".PWSTR,
    pszContainer: @"Windows.Win32.Foundation".PWSTR,
    pszCryptoProvider: @"Windows.Win32.Foundation".PWSTR,
};
pub const WLX_PROFILE_V1_0 = extern struct {
    dwType: u32,
    pszProfile: @"Windows.Win32.Foundation".PWSTR,
};
pub const WLX_PROFILE_V2_0 = extern struct {
    dwType: u32,
    pszProfile: @"Windows.Win32.Foundation".PWSTR,
    pszPolicy: @"Windows.Win32.Foundation".PWSTR,
    pszNetworkDefaultUserProfile: @"Windows.Win32.Foundation".PWSTR,
    pszServerName: @"Windows.Win32.Foundation".PWSTR,
    pszEnvironment: @"Windows.Win32.Foundation".PWSTR,
};
pub const WLX_MPR_NOTIFY_INFO = extern struct {
    pszUserName: @"Windows.Win32.Foundation".PWSTR,
    pszDomain: @"Windows.Win32.Foundation".PWSTR,
    pszPassword: @"Windows.Win32.Foundation".PWSTR,
    pszOldPassword: @"Windows.Win32.Foundation".PWSTR,
};
pub const WLX_TERMINAL_SERVICES_DATA = extern struct {
    ProfilePath: [257]u16,
    HomeDir: [257]u16,
    HomeDirDrive: [4]u16,
};
pub const WLX_CLIENT_CREDENTIALS_INFO_V1_0 = extern struct {
    dwType: u32,
    pszUserName: @"Windows.Win32.Foundation".PWSTR,
    pszDomain: @"Windows.Win32.Foundation".PWSTR,
    pszPassword: @"Windows.Win32.Foundation".PWSTR,
    fPromptForPassword: @"Windows.Win32.Foundation".BOOL,
};
pub const WLX_CLIENT_CREDENTIALS_INFO_V2_0 = extern struct {
    dwType: u32,
    pszUserName: @"Windows.Win32.Foundation".PWSTR,
    pszDomain: @"Windows.Win32.Foundation".PWSTR,
    pszPassword: @"Windows.Win32.Foundation".PWSTR,
    fPromptForPassword: @"Windows.Win32.Foundation".BOOL,
    fDisconnectOnLogonFailure: @"Windows.Win32.Foundation".BOOL,
};
pub const WLX_CONSOLESWITCH_CREDENTIALS_INFO_V1_0 = extern struct {
    dwType: u32,
    UserToken: @"Windows.Win32.Foundation".HANDLE,
    LogonId: @"Windows.Win32.Foundation".LUID,
    Quotas: @"Windows.Win32.Security".QUOTA_LIMITS,
    UserName: @"Windows.Win32.Foundation".PWSTR,
    Domain: @"Windows.Win32.Foundation".PWSTR,
    LogonTime: i64,
    SmartCardLogon: @"Windows.Win32.Foundation".BOOL,
    ProfileLength: u32,
    MessageType: u32,
    LogonCount: u16,
    BadPasswordCount: u16,
    ProfileLogonTime: i64,
    LogoffTime: i64,
    KickOffTime: i64,
    PasswordLastSet: i64,
    PasswordCanChange: i64,
    PasswordMustChange: i64,
    LogonScript: @"Windows.Win32.Foundation".PWSTR,
    HomeDirectory: @"Windows.Win32.Foundation".PWSTR,
    FullName: @"Windows.Win32.Foundation".PWSTR,
    ProfilePath: @"Windows.Win32.Foundation".PWSTR,
    HomeDirectoryDrive: @"Windows.Win32.Foundation".PWSTR,
    LogonServer: @"Windows.Win32.Foundation".PWSTR,
    UserFlags: u32,
    PrivateDataLen: u32,
    PrivateData: *u8,
};
pub const WLX_DESKTOP = extern struct {
    Size: u32,
    Flags: u32,
    hDesktop: @"Windows.Win32.System.StationsAndDesktops".HDESK,
    pszDesktopName: @"Windows.Win32.Foundation".PWSTR,
};
pub const WLX_DISPATCH_VERSION_1_0 = extern struct {
    WlxUseCtrlAltDel: ?*const anyopaque,
    WlxSetContextPointer: ?*const anyopaque,
    WlxSasNotify: ?*const anyopaque,
    WlxSetTimeout: ?*const anyopaque,
    WlxAssignShellProtection: ?*const anyopaque,
    WlxMessageBox: ?*const anyopaque,
    WlxDialogBox: ?*const anyopaque,
    WlxDialogBoxParam: ?*const anyopaque,
    WlxDialogBoxIndirect: ?*const anyopaque,
    WlxDialogBoxIndirectParam: ?*const anyopaque,
    WlxSwitchDesktopToUser: ?*const anyopaque,
    WlxSwitchDesktopToWinlogon: ?*const anyopaque,
    WlxChangePasswordNotify: ?*const anyopaque,
};
pub const WLX_DISPATCH_VERSION_1_1 = extern struct {
    WlxUseCtrlAltDel: ?*const anyopaque,
    WlxSetContextPointer: ?*const anyopaque,
    WlxSasNotify: ?*const anyopaque,
    WlxSetTimeout: ?*const anyopaque,
    WlxAssignShellProtection: ?*const anyopaque,
    WlxMessageBox: ?*const anyopaque,
    WlxDialogBox: ?*const anyopaque,
    WlxDialogBoxParam: ?*const anyopaque,
    WlxDialogBoxIndirect: ?*const anyopaque,
    WlxDialogBoxIndirectParam: ?*const anyopaque,
    WlxSwitchDesktopToUser: ?*const anyopaque,
    WlxSwitchDesktopToWinlogon: ?*const anyopaque,
    WlxChangePasswordNotify: ?*const anyopaque,
    WlxGetSourceDesktop: ?*const anyopaque,
    WlxSetReturnDesktop: ?*const anyopaque,
    WlxCreateUserDesktop: ?*const anyopaque,
    WlxChangePasswordNotifyEx: ?*const anyopaque,
};
pub const WLX_DISPATCH_VERSION_1_2 = extern struct {
    WlxUseCtrlAltDel: ?*const anyopaque,
    WlxSetContextPointer: ?*const anyopaque,
    WlxSasNotify: ?*const anyopaque,
    WlxSetTimeout: ?*const anyopaque,
    WlxAssignShellProtection: ?*const anyopaque,
    WlxMessageBox: ?*const anyopaque,
    WlxDialogBox: ?*const anyopaque,
    WlxDialogBoxParam: ?*const anyopaque,
    WlxDialogBoxIndirect: ?*const anyopaque,
    WlxDialogBoxIndirectParam: ?*const anyopaque,
    WlxSwitchDesktopToUser: ?*const anyopaque,
    WlxSwitchDesktopToWinlogon: ?*const anyopaque,
    WlxChangePasswordNotify: ?*const anyopaque,
    WlxGetSourceDesktop: ?*const anyopaque,
    WlxSetReturnDesktop: ?*const anyopaque,
    WlxCreateUserDesktop: ?*const anyopaque,
    WlxChangePasswordNotifyEx: ?*const anyopaque,
    WlxCloseUserDesktop: ?*const anyopaque,
};
pub const WLX_DISPATCH_VERSION_1_3 = extern struct {
    WlxUseCtrlAltDel: ?*const anyopaque,
    WlxSetContextPointer: ?*const anyopaque,
    WlxSasNotify: ?*const anyopaque,
    WlxSetTimeout: ?*const anyopaque,
    WlxAssignShellProtection: ?*const anyopaque,
    WlxMessageBox: ?*const anyopaque,
    WlxDialogBox: ?*const anyopaque,
    WlxDialogBoxParam: ?*const anyopaque,
    WlxDialogBoxIndirect: ?*const anyopaque,
    WlxDialogBoxIndirectParam: ?*const anyopaque,
    WlxSwitchDesktopToUser: ?*const anyopaque,
    WlxSwitchDesktopToWinlogon: ?*const anyopaque,
    WlxChangePasswordNotify: ?*const anyopaque,
    WlxGetSourceDesktop: ?*const anyopaque,
    WlxSetReturnDesktop: ?*const anyopaque,
    WlxCreateUserDesktop: ?*const anyopaque,
    WlxChangePasswordNotifyEx: ?*const anyopaque,
    WlxCloseUserDesktop: ?*const anyopaque,
    WlxSetOption: ?*const anyopaque,
    WlxGetOption: ?*const anyopaque,
    WlxWin31Migrate: ?*const anyopaque,
    WlxQueryClientCredentials: ?*const anyopaque,
    WlxQueryInetConnectorCredentials: ?*const anyopaque,
    WlxDisconnect: ?*const anyopaque,
    WlxQueryTerminalServicesData: ?*const anyopaque,
};
pub const WLX_DISPATCH_VERSION_1_4 = extern struct {
    WlxUseCtrlAltDel: ?*const anyopaque,
    WlxSetContextPointer: ?*const anyopaque,
    WlxSasNotify: ?*const anyopaque,
    WlxSetTimeout: ?*const anyopaque,
    WlxAssignShellProtection: ?*const anyopaque,
    WlxMessageBox: ?*const anyopaque,
    WlxDialogBox: ?*const anyopaque,
    WlxDialogBoxParam: ?*const anyopaque,
    WlxDialogBoxIndirect: ?*const anyopaque,
    WlxDialogBoxIndirectParam: ?*const anyopaque,
    WlxSwitchDesktopToUser: ?*const anyopaque,
    WlxSwitchDesktopToWinlogon: ?*const anyopaque,
    WlxChangePasswordNotify: ?*const anyopaque,
    WlxGetSourceDesktop: ?*const anyopaque,
    WlxSetReturnDesktop: ?*const anyopaque,
    WlxCreateUserDesktop: ?*const anyopaque,
    WlxChangePasswordNotifyEx: ?*const anyopaque,
    WlxCloseUserDesktop: ?*const anyopaque,
    WlxSetOption: ?*const anyopaque,
    WlxGetOption: ?*const anyopaque,
    WlxWin31Migrate: ?*const anyopaque,
    WlxQueryClientCredentials: ?*const anyopaque,
    WlxQueryInetConnectorCredentials: ?*const anyopaque,
    WlxDisconnect: ?*const anyopaque,
    WlxQueryTerminalServicesData: ?*const anyopaque,
    WlxQueryConsoleSwitchCredentials: ?*const anyopaque,
    WlxQueryTsLogonCredentials: ?*const anyopaque,
};
pub const WLX_NOTIFICATION_INFO = extern struct {
    Size: u32,
    Flags: u32,
    UserName: @"Windows.Win32.Foundation".PWSTR,
    Domain: @"Windows.Win32.Foundation".PWSTR,
    WindowStation: @"Windows.Win32.Foundation".PWSTR,
    hToken: @"Windows.Win32.Foundation".HANDLE,
    hDesktop: @"Windows.Win32.System.StationsAndDesktops".HDESK,
    pStatusCallback: ?*const anyopaque,
};
