...
pub const aliases = struct {
    ...
    pub const LPALLOCATEBUFFER = ?*const anyopaque;
    pub const LPALLOCATEMORE = ?*const anyopaque;
    pub const LPFREEBUFFER = ?*const anyopaque;
    pub const LPNOTIFCALLBACK = ?*const anyopaque;
    pub const LPFNABSDI = ?*const anyopaque;
    pub const LPFNDISMISS = ?*const anyopaque;
    pub const LPFNBUTTON = ?*const anyopaque;
    pub const CALLERRELEASE = ?*const anyopaque;
    pub const PFNIDLE = ?*const anyopaque;
    pub const LPOPENSTREAMONFILE = ?*const anyopaque;
    pub const LPDISPATCHNOTIFICATIONS = ?*const anyopaque;
    pub const LPCREATECONVERSATIONINDEX = ?*const anyopaque;
    pub const LPWABOPEN = ?*const anyopaque;
    pub const LPWABOPENEX = ?*const anyopaque;
    pub const LPWABALLOCATEBUFFER = ?*const anyopaque;
    pub const LPWABALLOCATEMORE = ?*const anyopaque;
    pub const LPWABFREEBUFFER = ?*const anyopaque;
};
